...
const authenticate = async (
    ...
    try {
        ...
        res.locals.role = (decodedToken as any).role || (decodedToken as any).admin ? 'admin' : res.locals.role;
        // Keep the full decoded token for downstream checks (custom claims, etc.)
        res.locals.decodedToken = decodedToken as any;
        next();
    } catch (error: unknown) {
